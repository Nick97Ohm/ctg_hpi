...
def parallel_sequential_generation(
    ...
    iteration_times, old_energy_times, mlm_times, disc_times, sampling_times, new_energy_times, meta_times = ([] for i in range(7))
    ...
    batch_original = torch.tensor(get_init_text(seed_text_src[:len(seed_text)]+['.']*(diff), max_len, batch_size)).to(device) #batch.detach().clone()
        ...
        for positions in groups:
            ...
            if args.degenerate: 
                ...
                if 1==1:
                    ...
                    mlm_1, mlm_2 = energy_score_mlm(batch,args.beta)
                    print("MLM")
                    print(mlm_1)
               ...
               #print("new dist\n",distance)
                ...
                for i in range(batch_size):
                    full_meta_data[i].append( (sentiment, r_score[i],norm_score[i],qxxb[i],qxbx[i],axbx[i],acc[i].item(),disc_preds[i], distance[i] ,bleurt[i], lexicon[i]))
...
times = []
iteration_times_set, old_energy_times_set, mlm_times_set, disc_times_set, sampling_times_set, new_energy_times_set, meta_times_set = ([] for i in range(7))
def generate(
    n_samples,
    dict_ag,
    model_disc,
    ...
    for batch_n in range(n_batches):
        ...
        if (batch_n + 1) % print_every == 0:
            ...
            print("Iteration Complete: Avg(%f), Var(%f), " % (np.mean(iteration_times_set), np.var(iteration_times_set)))
            ...
            print("Old Full Energy Score Complete: Avg(%f), Var(%f), " % (np.mean(old_energy_times_set), np.var(old_energy_times_set)))
            print("Sampling Complete: Avg(%f), Var(%f), " % (np.mean(sampling_times_set), np.var(sampling_times_set)))
            print("New Energy Score Complete: Avg(%f), Var(%f), " % (np.mean(new_energy_times_set), np.var(new_energy_times_set)))
...
tokenizer_disc = BertTokenizer.from_pretrained(args.disc_dir)  # finiteautomata/bertweet-base-sentiment-analysis ("textattack/bert-base-uncased-imdb")
...
if args.degenerate:
    folder_name = "mask_degenerate_topk_{}_burnin_{}_disc_{}_data_{}_max_iter_{}_temp_{}_shuffle_{}_block_{}_alpha_{}_beta_{}_delta_{}_gamma_{}_theta_{}_date_{}".format(top_k,burnin, args.disc_name,args.data_name,max_iter,temperature,args.shuffle_positions,args.block,args.alpha,args.beta,args.delta, args.gamma, args.theta ,dt_string)    
else:
    folder_name = "mask_disc_{}_data_{}_max_iter_{}_date_{}".format(args.disc_name,args.data_name,max_iter,temperature,args.shuffle_positions,args.block, args.alpha,args.beta, args.delta,args.gamma, args.theta, dt_string)
...
with open(f"{dirname}/samples.txt", "w") as f, open(f"{dirname}/opt_samples.txt", "w") as optimal_f,open(f"{dirname}/opt_cls.txt", "w") as optimal_class,open(f"{dirname}/opt_meta.txt", "w") as opt_meta_file ,open(f"{dirname}/metadata.txt", "w") as f_meta , open(f"{data_dir}", "r") as data_file,  open(f"{data_dir_src}", "r") as src_file, open(f"{attr_dir}", "r") as attr_file:
