...
def main():
    global parser, args, cuda, device, model, tokenizer, model_bleurt, model_bert
    ...
    for ii in range(max_iter):
        ...
        for positions in groups:
            ...
            if args3.degenerate:
                ...
                disc_1, disc_2, disc_preds = energy_score_disc(batch, model_disc=model_disc,
                                                               tokenizer_disc=tokenizer_disc, sentiment=sentiment,
                                                               alpha=args3.alpha)
                old_r, old_norm = np.array(energy_score_mlm(batch,beta=args.beta)) + np.array([disc_1, disc_2])
                old_r += args3.delta * distance
                old_r -= args3.gamma * bleurt_score
                # print(nmasks, positions, groups)
                old_wrd = batch[:, positions].detach().clone()
                ...
                for i in range(batch_size):
                    full_meta_data[i].append((sentiment, r_score[i], norm_score[i], qxxb[i], qxbx[i], axbx[i],
                                              acc[i].item(), disc_preds[i], distance[i], bleurt[i]))
...
def generate_samples(sentence):
    global args
    with open(f"{dirname}/samples.txt", "w") as f, open(f"{dirname}/opt_samples.txt", "w") as optimal_f, open(
            f"{dirname}/opt_cls.txt", "w") as optimal_class, open(f"{dirname}/opt_meta.txt",
                                                                  "w") as opt_meta_file, open(
        f"{dirname}/metadata.txt", "w") as f_meta:
        seed_text = sentence[:-1]
...
if __name__ == "__main__":
    ...
    parser, args, cuda, device, model, tokenizer, model_bleurt, model_bert = None, None, None, None, None, None, None, None
    main()
    ...
    tokenizer_disc = BertTokenizer.from_pretrained(
        args.disc_dir)  # BertTokenizer # finiteautomata/bertweet-base-sentiment-analysis ("textattack/bert-base-uncased-imdb")
    ...
    if args.degenerate:
        ...
        folder_name = "disc_{}_data_{}_max_iter_{}_date_{}".format(args.disc_name, args.data_name, max_iter,
                                                                   temperature,
                                                                   args.shuffle_positions, args.block, args.alpha,
                                                                   args.beta, args.delta, args.gamma, args.theta,
                                                                   dt_string)
